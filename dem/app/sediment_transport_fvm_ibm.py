...
class sediment_transport_app(base_app):
    def __init__(self, run_name="sediment_transport", slope=0.01, u_inlet=1.0):
        ...
        self.fluid_solver.turbulence_model = 'k-epsilon' # RANS k-epsilon
        ...
        self.fluid_solver.omega_t = np.zeros((self.nx, self.ny)) # Re-using omega_t to store epsilon
        ...
        self.fluid_solver.omega_t[:, :j_water] = eps_init
        # Set background small values
        self.fluid_solver.k[self.fluid_solver.k == 0] = 1e-6
        self.fluid_solver.omega_t[self.fluid_solver.omega_t == 0] = 1e-6
    ...
    def update(self):
        ...
        if self.fluid_solver.k is not None:
            ...
            self.fluid_solver.k[0, j_max:] = 1e-6
            self.fluid_solver.omega_t[0, :j_max] = eps_inlet
            self.fluid_solver.omega_t[0, j_max:] = 1e-6
        ...
        if self.fluid_solver.turbulence_model == 'k-epsilon' and self.fluid_solver.k is not None:
             solve_k_epsilon(self.fluid_solver.k, self.fluid_solver.omega_t, self.grid.nu_t, 
                             self.grid.u, self.grid.v, 
                             self.grid.nx, self.grid.ny, self.grid.dx, self.grid.dy, self.dt,
                             self.fluid_solver.mu_water/self.fluid_solver.rho_water, 
                             self.fluid_solver.C_mu, self.fluid_solver.sigma_k_eps, 
                             self.fluid_solver.sigma_eps, self.fluid_solver.C1_eps, self.fluid_solver.C2_eps)
    ...
    def plot(self, frame):
        ...
        # 5. Dissipation (Epsilon) (Adjusted Scale for Epsilon)
        self.ax_omega.clear()
        self.cax_omega.clear()
        if self.fluid_solver.omega_t is not None:
            # Use log scale for epsilon
            eps_plot = np.log10(self.fluid_solver.omega_t + 1e-10)
            im_omega = self.ax_omega.imshow(eps_plot.T, origin='lower', extent=[0, self.L, 0, self.H], cmap='magma', vmin=-6, vmax=-2)
            plot_particles(self.ax_omega)
            self.ax_omega.set_title("Dissipation Rate (log10 Epsilon) (m2/s3)")
            self.fig.colorbar(im_omega, cax=self.cax_omega)

def run_simulation(run_name, slope, u_inlet):
    app = sediment_transport_app(run_name, slope, u_inlet)
