...
class sediment_transport_k_omega_standard_app(base_app):
    def __init__(self, run_name="sediment_transport", slope=0.01, u_inlet=1.0):
        ...
        self.fluid_solver.turbulence_model = 'k-omega' # RANS k-omega Standard
        ...
        # Standard K-Omega requires non-zero omega in freestream to keep nu_t = k/omega small.
        # If omega is small, nu_t blows up. Set to a larger value (e.g. 10.0 or similar to inlet).
        self.fluid_solver.omega_t[self.fluid_solver.omega_t == 0] = 10.0
    ...
    def update(self):
        ...
        if self.fluid_solver.k is not None:
            ...
            self.fluid_solver.omega_t[0, j_max:] = 10.0
            ...
            # For now, let's keep top as is (Neumann-ish or fixed)
            # But ensure it's not zero
            self.fluid_solver.omega_t[:, -1] = 10.0
        ...
        if self.fluid_solver.turbulence_model == 'k-omega' and self.fluid_solver.k is not None:
             solve_k_omega(self.fluid_solver.k, self.fluid_solver.omega_t, self.grid.nu_t, 
                             self.grid.u, self.grid.v, 
                             self.grid.nx, self.grid.ny, self.grid.dx, self.grid.dy, self.dt,
                             self.fluid_solver.mu_water/self.fluid_solver.rho_water, 
                             self.fluid_solver.beta_star_kw, self.fluid_solver.alpha_kw, 
                             self.fluid_solver.beta_kw, self.fluid_solver.sigma_k, self.fluid_solver.sigma_w)
...
def run_simulation(run_name, slope, u_inlet):
    app = sediment_transport_k_omega_standard_app(run_name, slope, u_inlet)
    ...
    np.savetxt(f"flux_{run_name}_refined_k_omega_std.csv", data, delimiter=",", header=header, comments='')
